...
async function init() {
  ...
  console.log(`rsync ${__dirname}/mark-deletions.sh ${remote.fullpath}/.rawdb/bin/`)
...
async function fetch() {
  ...
  if (rsyncRemoteToLocal.status === 0) {
    ...
    const deletedFilesOnRemote = markDeletionsOnRemote()
    applyDeletionsFromRemote(deletedFilesOnRemote)
...
/** @param {string} deletedFilesOnRemote */
async function applyDeletionsFromRemote(deletedFilesOnRemote) {
  for (const toDelete of deletedFilesOnRemote.trim().split('\n').filter(Boolean)) {
