...
/** @type {(collection: rawdb.Collection, filename: string, includeBodySource?: boolean) => Promise<rawdb.CollectionItem | undefined>} */
export async function loadItem(collection, filename, includeBodySource = false) {
...
export async function applyItemChanges(collection, filename, changes) {
  if (!collection || typeof filename !== 'string') throw new TypeError(`bad arguments`)
  // console.debug('*', {collection.name, _id, changes})
  const item = await loadItem(collection, filename, true)
