  ...
  return (req, res) => {
    ...
    if (maybeId) {
      ...
      if (collectionName) {
        ...
        const item = state[collectionName].find(o => o._id === maybeId)
        res[stateResponseProperty][itemProperty ?? 'current'] = item
        res[stateResponseProperty]['currentCollection'] = collectionName
        res[stateResponseProperty]['currentItem'] = item
      }
    }
