...
import { loadSettings, getRootDirectory, getSetting } from './settings.js'
...
async function loadCollections(collectionEntries) {
  for await (const collectionEntry of collectionEntries) {
    ...
    const collection = {
      ...
      path: `${getRootDirectory()}/${collectionName}`,
...
async function loadCollectionData(collection) {
  ...
  console.log(collection.path, entries.length)
...
export function getCollectionsFlatten() {
  // TODO: Χρειάζεται στο middleware. Αν μείνει πρέπει να γίνει cached
  const flatten = {}
