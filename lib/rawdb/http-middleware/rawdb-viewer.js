...
async function rawdbRequestHandler(req, res, rawdbEndpoint) {
  ...
  if (section === 'collections') {
    if (sectionName) {
      ...
      if (!collection) return res.end(`collection "${sectionName}" not found`)
      return res.end(JSON.stringify({
        _itemsCount: collection.items.length,
        ...collection,
        items: `${url.origin}/rawdb/data/${sectionName}`,
        transformers: collection.transformers.map(fn => fn.toString())
      }))
    } else {
